...
inline void runCatRestPose() { 
  Serial.println(F("CAT REST")); 
  setFaceWithMode("rest", FACE_ANIM_BOOMERANG); 
  
  // Omuzlar (Ust eklemler) one ve arkaya kivrilacak
  // Stand'de L1=45, R1=135. Bunlari tam one (L1=0, R1=180) kiviriyoruz.
  // Arkalar L2=135, R2=45. Bunlari tam arkaya (L2=180, R2=0) kiviriyoruz.
  setServoAngle(L1, 0);   
  setServoAngle(R1, 180); 
  setServoAngle(L2, 180); 
  setServoAngle(R2, 0);   
  
  // Dizler (Alt eklemler) iceri (govde altina) kivrilacak
  setServoAngle(L3, 180); // Iceri
  setServoAngle(R3, 0);   // Iceri
  setServoAngle(L4, 0);   // Iceri
  setServoAngle(R4, 180); // Iceri
  
  if (currentCommand == "cat rest") currentCommand = "";
}

inline void runStargazePose() {
  Serial.println(F("STARGAZE"));
  setFace("excited");
  
  // Arka ayaklar oturur, ön ayaklar yukari kalkar (Göğe bakis)
  setServoAngle(L2, 135); setServoAngle(R2, 45); // Arka omuzlar normal
  setServoAngle(L4, 180); setServoAngle(R4, 0);  // Arka dizler tam yatik
  
  setServoAngle(L1, 0); setServoAngle(R1, 180);  // Ön omuzlar dik
  setServoAngle(L3, 135); setServoAngle(R3, 45); // Ön dizler düz
  
  delayWithFace(500);
  
  // Peri tozu / Yildiz sesi
  static const int starFreqs[] = {1568, 2093, 2637, 3136};
  static const int starDurs[]  = {100,  100,  100,  400};
  
  for (int i=0; i<4; i++) {
    tone(BUZZER_PIN, starFreqs[i], starDurs[i]);
    delayWithFace(starDurs[i] + 50);
  }
  
  delayWithFace(1000);
  runStandPose(1);
  if (currentCommand == "stargaze") currentCommand = "";
}
...
inline void runYawnPose() {
  Serial.println(F("YAWN"));
  setFace("sleepy");
  // Stretch arms and legs
  setServoAngle(L1, 0); setServoAngle(R1, 180);
  setServoAngle(L3, 180); setServoAngle(R3, 0);
  setServoAngle(L2, 180); setServoAngle(R2, 0);
  setServoAngle(L4, 0); setServoAngle(R4, 180);
...
inline void runSneezePose() {
  Serial.println(F("SNEEZE"));
  setFace("surprised");
  delayWithFace(200);
  
  // Head jerk down (Sneeze)
  setServoAngle(L1, 90); setServoAngle(R1, 90);
  tone(BUZZER_PIN, 3000, 100);
  delayWithFace(200);
  
  runStandPose(1);
  if (currentCommand == "sneeze") currentCommand = "";
}
...
inline void runWavePose() { 
  ...
  delayWithFace(300); 
  for (int i = 0; i < 4; i++) { 
    setServoAngle(L3, 180); delayWithFace(300); if (currentCommand == "") break;
    setServoAngle(L3, 100); delayWithFace(300); if (currentCommand == "") break;
  } 
  runStandPose(1); 
...
inline void runDancePose() { 
  ...
  for (int i = 0; i < 5; i++) { 
    ...
    setServoAngle(L3, 10); setServoAngle(L4, 10); 
    delayWithFace(300); if (currentCommand == "") break;
    setServoAngle(R4, 160); setServoAngle(R3, 160); 
...
inline void runSwimPose() { 
  ...
  for (int i = 0; i < 4; i++) { 
    ...
    delayWithFace(400); if (currentCommand == "") break;
    setServoAngle(R1, 90); setServoAngle(R2, 90); 
    setServoAngle(L1, 90); setServoAngle(L2, 90); 
    delayWithFace(400); if (currentCommand == "") break;
  } 
...
inline void runPushupPose() {
  ...
  for (int i = 0; i < 4; i++) {
    setServoAngle(L3, 0);
    setServoAngle(R3, 180);
    delayWithFace(600); if (currentCommand == "") break;
    setServoAngle(L3, 90);
    setServoAngle(R3, 90);
    delayWithFace(500); if (currentCommand == "") break;
  }
  runStandPose(1);
...
inline void runKneadPose() {
  Serial.println(F("KNEAD (HAMUR YOGURMA)"));
  setFaceWithMode("love", FACE_ANIM_BOOMERANG);
  runStandPose(0);
  delayWithFace(150);
  
  // Arka ayaklar dengeli ve hafif alcak
  setServoAngle(L2, 135); setServoAngle(R2, 45);
  setServoAngle(L4, 180); setServoAngle(R4, 0);
  setServoAngle(L3, 0);   setServoAngle(R3, 180);
  
  // On patiler sirayla hamur yogurur
  for (int cycle = 0; cycle < 3; cycle++) {
    // Sag pati basar, sol pati kalkar
    setServoAngle(R1, 155);
    setServoAngle(L1, 25);
    delayWithFace(280);
    if (currentCommand == "") break;
    
    // Sol pati basar, sag pati kalkar
    setServoAngle(R1, 115);
    setServoAngle(L1, 65);
    delayWithFace(280);
    if (currentCommand == "") break;
  }
  ...
  runStandPose(1);
  if (currentCommand == "knead") currentCommand = "";
}
